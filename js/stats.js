  ...
  computed: {
    frequencyTable() {
      ...
      return Object.keys(counts).map(num => {
        ...
        return { number: num, count, style: { backgroundColor: bg, color: text } };
      }).sort((a, b) => {
    ...
    sumStats() {
      ...
      this.sortedResults.forEach(r => {
        ...
        if (!stats[interval]) stats[interval] = { label: `${interval}-${interval + 19}`, count: 0, start: interval };
        stats[interval].count++;
      });
      const max = Math.max(...Object.values(stats).map(s => s.count));
      return Object.values(stats)
        .map(s => ({ ...s, percentTotal: ((s.count / total) * 100).toFixed(1), percentBar: (s.count / max) * 100 }))
